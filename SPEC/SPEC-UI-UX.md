...
- Layout: top bar "To review" + count · permission banner (conditional) · **suggestion cards**
  ...
  pinned at the bottom when the list is long, above it **Save all** (CR-12: saves every pending
  item with the Settings › Default category; see §6.14). No **confidence markers** (many auto-detection
...
- Controls: card body → Confirmation sheet; inline **`Save`** on a known-account card →
  ...
  all**; **Save all** → with a default set, a confirm dialog "Save N transactions?" then one
  batch save and a count toast (expenses take the account's learned category, else the default;
  income stays Uncategorized; no Undo — CR-12), with no default set, opens Settings › Default
  category.
...
- **Default category (P1, CR-12):** a single-select list of every category except the system *Uncategorized* row (that is what **None** means) plus **None**, with the
  current choice ticked; picking a row saves it and goes back. The Settings row shows the chosen
  name, or "Not set". Feeds the Review Queue's **Save all** (§6.3).
...
- **CR-9** (2026-09-19, phase 3 — `SPEC/SPEC-implementation.md` §45.8) — **Split UI as built: four small simplifications.** (1) **Transaction Details (§6.8):** there is no overflow menu on that screen (V1 has only a delete icon), so **Split…** is a row under the description (shown for an unsplit debit) and **Edit split** / **Remove split** live in the Split card's footer (Remove asks first: "Remove split?"); a person's tap expands *Waive — I'll cover it* / *Restore this share*. (2) **Split sheet (§6.17):** the **Contacts** row is a quiet placeholder ("Arrives with split requests — for now, add a number") until phase 5 wires contacts access; the primary button on the Amounts stage reads **Done** (returns to the Confirm / Edit sheet, which saves the split with the transaction) or **Save split** (opened from Details, saves immediately). "Send requests" arrives in phase 5. (3) **The Split row is a Confirm / Edit sheet row that works before the transaction exists** — in Confirm the transaction is not saved yet, so the split is held as a draft and written together with the transaction when the sheet is saved; Cancel discards both. A changed split counts as unsaved input for that sheet's swipe / Cancel guard (V-6). (4) **Deferred:** the Filter sheet's **Splits** chip (§6.9) and *Mark as paid…* / *Resend request* on a person (they need Merge / SMS) — they arrive with phases 4–5. Money in the new UI uses a **plain magnitude** (`₹300`, no `+`/`−`) via `formatRupees`; only signed transaction amounts keep the sign.

- **CR-10** (2026-09-20, phase 4 — `SPEC/SPEC-implementation.md` §45.9 / CR-22) — **Settlements & the Splits page as built: seven small decisions.** (1) **Merge from an item (§6.20):** a settlement always names a *transaction* (D37), so **Mark all paid…** (a person) and **Merge…** (a request) open the Merge sheet the other way round — a single-choice list of unmatched payments (a credit for shares, a debit for requests; an exact-amount one is pre-picked) titled **Mark as paid** / **Record payment**. With none available it says to add the payment as a transaction first. Starting from a payment (Details › **Merge into a split…**) is the checklist described in §6.20, unchanged. (2) **Undo** is the app's snackbar, held for 5 s (the same window as the delete Undo; the default 3 s was too short on a real phone); because that is still brief, each line of a payment's new **Settlements** section on Details (§6.8) has a **Remove**. (3) **Suggested settlement in the Confirmation sheet:** the transaction does not exist yet, so **Settle** only *marks the choice* (the card becomes "Will settle Rahul's share · when you save" with **Undo**); the settlement is written right after Save. **Not this** hides the card for that visit. (4) **Splits › Owed to you** group action is a single **Mark all paid…** button (no ⋯ menu — same simplification as CR-9); **Send reminder** arrives with messaging (phase 5). (5) **Requests** is list-only in this phase (Unattended / Accepted); Accept / Reject and the notification arrive in phase 5. The segment label carries the Unattended count as `Requests · 2` instead of a badge. (6) **Home** shows the **Owed to you ₹N** row directly under the Income / Spending tiles. (7) **"You owe" testing:** requests only arrive over SMS (phase 5), so a **dev build** shows *Add sample request (dev build only)* on that segment; it is gated by `__DEV__` and never ships.

- **CR-11** (2026-09-20, phase 5 — `SPEC/SPEC-implementation.md` §45.10 / CR-23) — **Messaging as built: eight small decisions.** (1) **Sending from the Confirm / Edit sheets (§6.17):** those sheets own the save, so the requests go out *after* their Save and the outcome arrives as a **toast** (`Requests sent to 2 people` / `1 of 2 requests failed — retry from Details`); the per-person **result list** appears only when the Split sheet itself saved (from Details). The small text button reads **Don't send now** (and **Send requests when saved** once off) and is hidden entirely where the device cannot send. (2) **Retrying** is per person, from the result list (**Retry**) or from Details — tapping a person on the Split card now expands **Send request** / **Retry request** / **Send a reminder** / **Resend — the amount changed**, next to Waive. (3) **SEND_SMS refused:** each recipient gets an **Open in Messages** action in the result list (one at a time, as decided) instead of the app opening them in a row; the line then reads *Opened in Messages*. (4) **Requests segment (§6.19):** an Unattended row has **Reject** · **Accept**; *both* get the Undo snackbar (the spec only promised one for Reject) since Accept is equally one tap. The row shows the masked number and *not in your people* for an unknown sender, matching the notification body. (5) **Home (§6.2):** the "requests waiting" badge §6.21 promises is an action-strip row — **N split requests** → Splits › Requests — shown only when N > 0, under **Owed to you**. (6) **Contacts (§6.17):** the section appears in place of the "Choose from contacts" row once access is granted; a blocked permission opens system settings, and an unsupported device says so. Contacts with no usable Indian mobile number are not listed. (7) **Settings › Splits & people** (§6.19 entry): *Your name in requests* (a single field, ≤ 20 GSM-7 characters, with a live echo of the sentence it produces), *Contacts access*, a **read-only** *Sending requests* row (default SIM only — D41, so there is nothing to choose), and *Saved people* (rename in place; **Remove** refuses while they are in a split and says why). (8) The Split sheet's **Cancel** disappears while the result list is up — the split is already saved by then, so **Done** is the only way out.

- **CR-12** (2026-09-20, post-V2-phase-6 — `SPEC/SPEC-implementation.md` CR-25) — **Default category + "Save all with default" in the Review Queue (§6.3, §6.14).** (1) **Settings › Default category** (new subpage, §6.14): pick one existing category, shown as a single-select list; a *Clear* row removes it. Stored as the `defaultCategoryId` setting; a default whose category was deleted is treated as unset. (2) **Review Queue (§6.3):** a **Save all** button sits beside **Dismiss all** in the footer whenever the queue is non-empty. With no default set, tapping it opens Settings › Default category instead. With one set, a confirm dialog reads "Save N transactions as <Category>?"; on confirm every pending item is saved and a toast reports the count. (3) **Rules:** expenses take the account's *learned* category when its account rule has one, otherwise the default; income is saved Uncategorized (IMP-011 — income carries no category, so there is no separate income default); bulk save never teaches account rules a category. Items with no amount, direction or time stay in the queue and the toast says how many were skipped. (4) **No Undo:** the Undo snackbar holds a single transaction id, so a bulk save is reversed by deleting the transactions individually; the confirm dialog is the safeguard.
- **CR-13** (2026-09-20, on-device — `SPEC/SPEC-implementation.md` CR-26) — **SMS-app fallback shows the system chooser.** In §6.17, when SEND_SMS is denied and a request is opened "in the SMS app", Android may first show an **"Open with" chooser** — on the Motorola edge 60 pro it lists **WhatsApp and Messages** for the `sms:` link. That is the OS's behaviour, not the app's; the app still pre-fills the text and never sends it. The user should pick their SMS app; picking another app is theirs to own. No design change.
