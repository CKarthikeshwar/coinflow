...
- **CR-21** (2026-09-19, phase 2 — linked UI-UX CR-8) — **Only a debit can be split; analytics as built.** (1) `createSplit` refuses a credit (`RangeError: only a debit … can be split`) — a share means "what someone owes you", which has no meaning for an income (see UI-UX CR-8); credits still *settle* shares. (2) §41's SQL is implemented as `effectiveAmountSql` in `src/db/repositories/analytics.ts` with **fully qualified table names** — Drizzle omits the table qualifier in single-table selects, and inside the correlated sub-selects an unqualified `id` silently bound to `split.id` (found by the real-SQL tests; a mocked test could not have shown it). (3) The "Biggest expenses" rows return the transaction with `amountMinor` **replaced by the effective amount**, so ranking, the shown figure and the Spent total agree. (4) Category breakdown, Biggest expenses and the daily series drop expenses whose effective amount is 0 (a fully covered expense does not appear as a ₹0 row and does not make the previous period "have data").
- **CR-22** (2026-09-20, phase 4 — linked UI-UX CR-10) — **Settlements & the Splits page as built.** Adds `listPaymentCandidates` (settlements repo), `listSettledShares` / `groupOwedByPerson` (splits), `listSettledRequests` (split-requests) and the live hooks `useSplitsOverview` / `useSettlementsForTransaction` (§43.1 is otherwise unchanged); the `merge` sheet takes `{ transactionId }` **or** `{ shareId | requestId | personId }` (§43.2 said `{ transactionId | shareId | requestId }`; `personId` is *Mark all paid…*). No schema change.
- **CR-23** (2026-09-20, phase 5 — linked UI-UX CR-11) — **Messaging as built.** (1) The notification **category id is `splitRequest`**, not `split-request` (§43.4) — camelCase like the existing `txnKnown` / `txnNew`. (2) The **notification is posted by the service layer** (`src/services/splits/receive-request.ts`), not by `receiveRequest()` as §42.2(2) reads: repositories in this codebase stay synchronous and side-effect-free, and the same rule lets the app-open sweep record a request without announcing it (`notify: false`). (3) `sendSmsAsync` **resolves `'failed'` and never rejects** — a missing permission, an exception and the 30 s timeout are all data for the UI. (4) `RequestView` gains `knownPerson` (the sender is a saved person, not one auto-created from the SMS) for the "not in your people" label. (5) `PendingSplit` gains `send`, and `persistSplitDraft` returns the `splitId` so the caller can send for it. (6) `expo-contacts` is wrapped in `src/services/contacts.ts` (+ `.web.ts`) using the SDK-57 **legacy** API (§45.5), and `WRITE_CONTACTS` — which the package's plugin adds — is removed with `android.blockedPermissions` (IMP-097). (7) `TextField` gained `autoFocus` / `onBlur` (its own focus tracking still runs). No schema change.
- **CR-24** (2026-09-20, phase 6) — **Widgets as built.** (1) The snapshot bridge is `publishWidgetSnapshot(json): boolean` on the module (synchronous `Function`, never throws), reached through `src/services/sms.ts` (`publishWidgetSnapshotJson`, + `.web.ts` stub) — `src/` may only import the native module there. (2) Publishing is `src/services/widgets/publish.ts`: `computeWidgetSnapshot` (uses `periodSummaryQuery` — the Analytics card's own query — so Balance = Income − Spent by construction), `publishWidgetSnapshotNow`, debounced `schedulePublish` (1 s). (3) Triggers: the always-mounted `WidgetSync` (`features/app-shell/widget-sync.tsx`) watches transactions, suggestions, the split/settlement signal and `widgetHideAmounts`, plus every return to the foreground; the headless SMS ingest, the store-watcher task, the periodic sweep and the notification-response task (Save / Discard / Accept / Reject) publish when they finish. There is no per-repository write hook — table watching replaces the "repository writes" trigger in §44.3. (4) Each widget is one `RemoteViews` layout; the resize variant is chosen from the launcher-reported size and shown by toggling visibility (summary < 200 dp wide drops Income; queue ≥ 150 dp tall shows 3 rows, else 2; quick add ≥ 110 dp wide shows the label). (5) Palette: tile `#0B0B0C` / text `#F5F5F2` are the approved widget values, hairline / text2 / text3 come from `theme.ts`; native fonts are the system sans (Manrope / Geist are not bundled natively). (6) `coinflow://review` was only ever described, never a route — `src/app/review.tsx` (redirect to the Review Queue, opening the Confirmation sheet for `?open=<id>` when that suggestion is still pending) and `src/app/add.tsx` (Home + Add sheet) now exist. (7) Settings › **Widgets** (`src/app/widgets.tsx`, + `.web.tsx`). No schema change; no new permission (IMP-097).

- **CR-25** (2026-09-20, linked UI-UX CR-12) — **Default category + bulk save.** New setting `defaultCategoryId` (string | null; added to `SettingKey`). New `handleSaveAll(defaultCategoryId)` in `src/services/notifications/respond.ts` writes every complete pending suggestion in **one** `db.transaction` (insert transaction + mark suggestion confirmed, no account-rule writes), then cancels the answered notifications; returns `{ saved, skipped }`. Category per row: credit → `null` (IMP-011); debit → the account rule's `categoryId` if set, else the default. New route `src/app/default-category.tsx` (+ `.web.tsx`), a Settings row, and a `defaultCategoryId` that no longer matches a category is treated as unset (no `deleteCategory` change); `resolveDefaultCategory` also rejects the system *Uncategorized* row, and the picker hides it. No schema change; no new permission.
- **CR-26** (2026-09-20, on-device verification — linked UI-UX CR-13) — **Two-phone results and corrections.** (1) The Motorola's number is **9742590888** (the spec's earlier "second phone 9845897555" was an old SIM); the Samsung is 9390787053. (2) A valid **SEND_SMS denial test** needs `pm revoke` **and** `pm set-permission-flags --user 0 <pkg> android.permission.SEND_SMS user-fixed`: on the Motorola a plain revoke is silently re-granted when the app asks, with no dialog. (3) With the permission denied the reminder/resend path falls back to an `sms:` intent; on the Motorola the OS shows an **"Open with" chooser listing WhatsApp and Messages** (UI-UX CR-13). (4) The notification **Accept/Reject buttons did nothing on the debug dev client** (see §45.10); the shade also **groups CoinFlow requests under a "N split requests" summary that must be expanded, then each request expanded, to reach its buttons** on One UI. (5) Verified results are recorded in `SPEC/traceability.md` ("On-device verification — phase 5 … 2026-09-20"). No code change.

### 45.7 As built — phase 2 (effective-amount analytics), 2026-09-19

- **One expression, everywhere.** `effectiveAmountSql` (exported) = `MAX(0, CASE type WHEN 'expense' THEN amount − Σ non-waived shares WHEN 'income' THEN amount − Σ share-settlements ELSE amount END)`. Used by: `runningBalanceQuery` (all-time balance), `periodSummaryQuery` (Spent / Income — also feeds the Analytics arc, `useMoMDeltas`), `categoryBreakdownQuery`, `largestExpensesQuery`, `dailyExpenseRowsQuery` (feeds the daily series, mean / median and their previous-period figures). Week mode and every "previous period" comparison reuse the same queries, so they are consistent by construction.
- **Not money-based, so unchanged:** `useUncategorizedCount` (counts transactions).
- **A debit that settles a request you owe is not reduced** (only share-settlements net income; a request payment is your own expense).
- **Testability:** each query is now an exported factory (`periodSummaryQuery(period)` …) and its hook is a thin `useLiveQuery` wrapper with the same return shape, so the screens are untouched.
- **V1 parity proof:** `analytics.test.ts` re-implements the V1 SQL verbatim and asserts equal output on a 60-row varied dataset (expenses / incomes, categories, days, an older month, a soft-deleted row, a reserved `transfer` type) with no splits, in addition to the V1 domain suite passing unchanged.
- **Scope note:** `useOwedToYou` / `useYouOwe` live hooks (Home "Owed to you", Splits page) are phase 4; the repository functions `owedToYouMinor()` / `youOweMinor()` already exist from phase 1.

### 45.8 As built — phase 3 (Split UI), 2026-09-19

- **Pure engine** `src/domain/split-draft.ts` — `computeSplitDraft({ totalMinor, personKeys, mode, overrides })` → rows (You first), `remainingMinor`, `balanced`, `zeroPeople`, `canSave`; equal by default with leftover to You, custom rows fixed and the rest re-sharing equally, ₹ and % modes (% via `percentToMinor`), plus `parseRupeesToMinor` / `parsePercent` / `minorToRupeeText` / `percentText`. `src/domain/split-badge.ts` — `buildSplitBadges` / `splitBadgeLabel` for list rows.
- **State** `src/stores/split-draft.ts` (`useSplitDraft`): `committed` (what the parent will write) + the sheet's working state; `seedExisting`, `begin`, `togglePerson`, `setOverride`, `commit`, `removeSplit`, `pending()`, `dirty`. Registered in the barrel; reset by `SheetHost` whenever no sheet is open, by the parent's Save and by its discard.
- **Write path** `src/features/splits/persist-split.ts` — `persistSplitDraft(transactionId, pending, { direction })`: create / `replaceShares` (keeps each person's row + settlements) / `removeSplit`; ignores a credit (drops a leftover split). Called by `TransactionSheetBody` *after* the transaction is saved — a failing split write shows "Saved, but the split could not be saved" and never loses the transaction — and by the Split sheet itself in `direct` mode (Details).
- **UI** `src/features/splits/split-sheet.tsx` (People → Amounts; registered as sheet `'split'`, 92 % snap, returns to its parent like the category picker), `split-card.tsx` (Details card + status chips), `TransactionSheetBody` gains the **Split…** / **Split with N · ₹X yours** row (Confirm / Edit, debit, amount > 0) and the **IMP-089** rule (others' shares fixed, yours recomputed; Save blocked with "Others' shares are more than the amount — edit the split."). Details: share line ("Your share ₹300 · ₹500 still owed"), **Split…** row, card with Waive / Restore, Edit / Remove. `TransactionCard` takes an optional `split` badge; the Transactions tab and Home "Recent" pass it from `useSplitBadges()`.
- **Live reads** `src/db/repositories/split-hooks.ts` — `useSplitForTransaction(id)` and `useSplitBadges()` (subscribe to the small split tables and derive in JS).
- **New helper** `formatRupees(minor)` (unsigned) in `domain/format/money.ts`; new icon `users`.
- **Tests:** +107 (966 total): engine 31, store 13, persist 9, badge 9, Split sheet 19, Confirm/Edit sheet split cases 9, Details 7, card 3 (plus existing suites unchanged).
- **Not in phase 3:** Contacts access, sending, Merge / *Mark as paid…*, Splits page, Home "Owed to you" (phases 4–5); Filter › Splits chip.

**On-device verification (2026-09-19, Samsung SM-S711B, dev build, driven over adb):** add ₹1,200 → Details → Split… → add two people by number → equal ₹400 → custom ₹600 (others re-share to ₹300) → Save split → card, share line, list badge → waive a share → Remove split. Four defects were found *by doing this* and fixed (each has a regression test):
1. **Home / Analytics did not refresh after a split change** (phase 2 regression): Drizzle's `useLiveQuery` re-runs only when the query's *main* table changes, but `effectiveAmountSql` also reads the split tables. Every effective-amount hook now depends on `useSplitChangeSignal()` (`analytics.ts`, tested in `analytics-hooks.test.tsx`). Verified live: waiving a ₹600 share moved Balance from −₹300 to −₹900 with no restart.
2. **The number keyboard covered the "Add" button** in the Split sheet: the add-number form now sits under the search field and the keyboard's ✓ adds the person (`TextField` gained `returnKeyType` / `onSubmitEditing`).
3. **"Your share" ignored waived shares** (showed ₹300 while ₹900 was being spent): Details, the card's You row, the list badge and the Edit-sheet row now all use *what it really cost you* (amount − shares others still owe).
4. **The unsplit "Split…" row on Details was indented** past the text above it (aligned).

### 45.9 As built — phase 4 (Settlements & the Splits page), 2026-09-20

- **Data reads** — `splits.ts`: `listSettledShares()`, `groupOwedByPerson()` (pure; `listOwedByPerson` now uses it); `split-requests.ts`: `listSettledRequests()`; `settlements.ts`: `listPaymentCandidates(direction, limit = 60)` — live transactions of that direction with an unallocated remainder, newest first (the allocation sum is a separate grouped query: a correlated subquery lost its table qualifier in Drizzle and matched the wrong `id`).
- **Live reads** — `split-hooks.ts`: `useSplitsOverview()` (owed groups, settled lists, both totals, unattended / accepted requests, `ready`) and `useSettlementsForTransaction(id)` (lines + allocated). Same pattern as phase 3: subscribe to whole tiny tables (plus the *deleted* transactions, so deleting a shared transaction drops its shares) and derive in JS.
- **Merge sheet** `features/splits/merge-sheet.tsx` (sheet `'merge'`, 92 % snap, never dirty). *From a payment:* candidates = open shares (credit) / accepted requests (debit); `suggestSettlement` orders them and pre-ticks `bestId`; each ticked row has a ₹ field (default = what is owed) and the plan is `allocate(txn.amount, alreadyAllocated, picks)` in display order — so caps, the "₹X of ₹Y used" footer and the write can never disagree. *From an item:* single-choice list of `listPaymentCandidates`; picks are that person's / that request's open items oldest first, capped by `settle`.
- **Performing a merge** `features/splits/settle-and-announce.ts` — `settle()` (one DB transaction, IMP-083) then the `Settled Rahul's share — ₹450` / `Paid Priya's request — ₹450` / `Settled 2 shares — ₹900` snackbar whose **Undo** calls `unsettle(ids)`. A failure shows "Could not settle that" and writes nothing. Used by the Merge sheet, the Details banner and the Confirm sheet.
- **Suggested settlement** `features/splits/suggested-settlement-banner.tsx` — `useSettlementSuggestion(openShares, { availableMinor, account, isCredit })` (thin wrapper over `suggestSettlement`, credits only) + the card. Details: shown for a credit while one open share matches what is left of it; **Settle** settles immediately. Confirm sheet: reads open shares once, follows the draft's amount / account, and settles *after* the save (UI-UX CR-10 (3)); nothing is ever written without the user pressing **Settle** (IMP-085).
- **Details** `[id].tsx` — **Merge into a split…** row (credit with open shares, or debit with open requests, and something left to allocate); **Settlements** section `settlements-section.tsx` (per line **Remove** → `unsettle`, remainder "₹150 of this payment isn't assigned to anything").
- **Splits page** `src/app/splits/index.tsx` (+ `.web.tsx` notice; registered in `root-navigator.tsx`): three segments, `?tab=requests|owe`, grouped rows with `StatusChip`, **Show settled (N)**, **Mark all paid…**, per-row **Merge…** on You owe, skeleton until `ready`, empty copy per §6.19. `coinflow://splits` needs no code — the `coinflow` scheme + the route file are enough for expo-router. Home: `features/home/owed-row.tsx`.
- **Dev fixture** `features/splits/dev-seed.ts` — `addSampleRequest()` (receive + accept from a rotating sample sender); button only under `__DEV__`.
- **Tests:** +62 (1,035 total): repo lists 5, hooks 5, settle-and-announce 4, Merge sheet 11, Splits page 13 + dev-seed 2, Details 8, Confirm-sheet banner 5, Home 2 (some existing suites gained mocks). Typecheck, lint clean.
- **Not in phase 4:** Accept / Reject and *Send reminder* (phase 5), Filter › Splits chip, Settings › Splits & people (phase 5).

**On-device verification (2026-09-20, Samsung SM-S711B, dev build, driven with Maestro — `e2e/v2-p4-*.yaml`):** split a ₹1,200 debit (two people, ₹400 each) → Home shows **Owed to you ₹800** and the split badge → added a ₹400 credit → with *two* equal ₹400 shares there is correctly **no** suggestion (ambiguous), **Merge into a split…** opens the Merge sheet (people grouped, "₹0 of ₹400 used", Settle disabled) → ticked one, **Settle** → Details shows **Settlements** with a **Remove**, and the merge row is gone once the payment is fully assigned → a second ₹400 credit from "NEHA" now matches exactly one open share → the **Suggested settlement** banner appears → **Settle** → snackbar → **Undo** → banner is back → **Remove** on a settlement → banner is back → Splits page: **Owed to you** (grouped, Pending chip, Show settled (1), Mark all paid…), **You owe** (dev sample request → "₹450 you owe" + Merge…), **Requests** (Accepted) → **Merge…** → "Record payment" list → picked the ₹1,200 debit → **Settle** → "Paid … request — ₹450", the request moved to *Show settled*. One defect found *by doing this* and fixed: **the Undo snackbar (3 s) expired before a real tap could reach it — the tap fell through to the Edit button underneath**; settle snackbars now stay 5 s (`useToast.show`'s new `durationMs`; regression test in `settle-and-announce.test.ts`). Not exercised on the phone: the Confirm-sheet banner (unit-tested), a typed amount in the Merge sheet, "Mark all paid…" with a matching payment.

### 45.10 As built — phase 5 (Messaging), 2026-09-20

- **Receive** `src/services/splits/receive-request.ts` — `handleIncomingRequest({ sender, body }, { notify })`, called by `smsIngestTask` **before** the sender gate from every inbound path (broadcast, store watcher, both sweeps). Returns `true` when the message was a request, and ingest stops there — so a request is never also a bank suggestion, and a bank sender (always alphanumeric) can never enter the branch. A request the rules refuse (duplicate, rate-limited, withdrawn-unknown) still counts as handled; a *malformed* token or an out-of-bounds amount falls through to the V1 path untouched.
- **Notifications** `notifications/split-content.ts` (title `Rahul requested ₹450`, body `for Momos · CoinFlow split[ · not in your people]`, id `req:<id>`), `split-post.ts` (post / `N split requests` summary / cancel, all silent without permission), `respond-split.ts` (headless **Accept** / **Reject**, each re-reading the row so a stale press is a no-op). New channel `split-requests` (DEFAULT importance) and category `splitRequest`; `NOTIFICATION_RESPONSE_TASK` routes `ACCEPT` / `REJECT` next to `SAVE` / `DISCARD`. Stale taps go through `resolveNotificationTarget`: unattended → Requests, accepted → You owe, decided/withdrawn → Splits, purged → Home.
- **Native** `SmsSender.kt` — `SmsManager.sendTextMessage` on the **default SMS subscription resolved at send time** (never cached — it differs per device, §45.5), a one-shot `sentIntent` receiver, 30 s timeout, outcome `"sent" | "failed"`, nothing logged but the exception class. Module gains `sendSmsAsync` / `getSendSmsPermissionAsync` / `requestSendSmsPermissionAsync`; the config plugin adds `SEND_SMS`; `app.json` adds the `expo-contacts` plugin and blocks `WRITE_CONTACTS`.
- **Send** `src/services/splits/send-requests.ts` — split written first, then one recipient at a time, each result stamped on its share as it lands (`requestState` / `requestSentAt` / `requestedAmountMinor`), so an app kill mid-run leaves an accurate record. `SEND_SMS` is asked for just-in-time; refused ⇒ **nothing is sent silently** and the recipients come back as `fallback` for `openInSmsApp` (`sms:<phone>?body=…`, recorded `opened_in_sms_app`). `needsRequest` / `canRequest` decide who is asked; `summarizeReport` words the toast.
- **UI** Split sheet: real **Contacts** section (just-in-time access, blocked ⇒ system settings), **Send requests** / **Don't send now**, and the result list with **Retry** / **Open in Messages**. Details: send / retry / reminder per person on the Split card. Splits › Requests: **Accept** / **Reject** with Undo, masked number, "not in your people". Home: **N split requests** row. New screen `src/app/splits-people.tsx` (+ `.web.tsx`, registered in `root-navigator.tsx`, linked from Settings).
- **Tests:** +85 (1,120 total): receive branch 14, ingest branch 5, notifications 11, send orchestration 15, Split sheet contacts + sending 12, Splits page requests 5, Details sending 5, Settings screen 7, deep-link 4, Home 1, plus updated existing suites. Typecheck and lint clean.
- **Not in phase 5:** the widgets (phase 6). Filter › Splits chip is still deferred.

**On-device verification: done 2026-09-20** on a Samsung SM-S711B (Jio) and a Motorola edge 60 pro (VIL), debug dev client, Metro running. Verified: send, receive (with the app closed, ~2 s), Unattended, Accept, pay, Merge/Settle, repeated sends of one ref collapsing to one row, and SEND_SMS denied ⇒ the SMS-app fallback (nothing sent, text pre-filled, not sent). A bank SMS became a suggestion, never a request. Swipe-away keeps a request Unattended (Pass) and in-app Reject works (Pass). **Not verified:** the notification's Accept/Reject buttons — on the debug dev client the tap is delivered (`actionIdentifier: ACCEPT`, right `requestId`) and `coinflow.NOTIFICATION_RESPONSE` starts and finishes ~3 ms later with no state change and no JS log, consistent with the phase-0 warning; re-check on a release-style build, and treat a repeat as a bug. **Note:** the phase-0 warning that the debug client does not reliably host headless JS did not bite here (Metro was running); a release-style build is still the safer check for the receive branch.

### 45.11 As built — phase 6 (Widgets), 2026-09-20

- **Native** (`modules/coinflow-sms`): `WidgetStore.kt` (snapshot in `SharedPreferences("coinflow_widgets")`, parse, `refreshAll`), `CoinflowWidgetProviders.kt` (`SummaryWidgetProvider`, `QueueWidgetProvider`, `QuickAddWidgetProvider`; Indian-grouped ₹, `••••` masking, stale month → `—` with the current month name, taps as `ACTION_VIEW` `PendingIntent`s with `FLAG_IMMUTABLE`), `res/layout/cf_widget_*.xml` (+ `_preview` layouts for the picker), `res/xml/cf_widget_*_info.xml` (`updatePeriodMillis` 30 min, summary resizable 4×2 ↔ 2×2, queue 4×2 ↔ 4×3 vertically, quick add 1×1 ↔ 2×1), `res/values/cf_widget_colors.xml` / `cf_widget_strings.xml`, drawables. Three `<receiver>`s are added by `app.plugin.js` (verified in the generated manifest after `prebuild --clean`).
- **JS:** `publish.ts`, `widget-sync.tsx`, `add.tsx`, `review.tsx`, `widgets.tsx` (see CR-24).
- **Tests:** publish 8 (figures, month bounds, pending + hide, native failure, debounce), deep-link redirects 4 → the whole suite passes; typecheck and lint clean.
- **On-device verification: partial** — the rebuilt client is installed and the user checked the three widgets visually on the Samsung (2026-09-20); still to do: add each widget, both sizes, tap targets (incl. a cold start from Quick add and from a queue row), Hide amounts, a real SMS updating the queue widget with the app closed, and a month rollover (or a simulated date).
