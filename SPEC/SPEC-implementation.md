...
**On-device verification: done 2026-09-20** on a Samsung SM-S711B (Jio) and a Motorola edge 60 pro (VIL), debug dev client, Metro running. Verified: send, receive (with the app closed, ~2 s), Unattended, Accept, pay, Merge/Settle, repeated sends of one ref collapsing to one row, and SEND_SMS denied ⇒ the SMS-app fallback (nothing sent, text pre-filled, not sent). A bank SMS became a suggestion, never a request. **Not exercised:** swipe-away keeps Unattended, Reject, and the notification's Accept/Reject buttons. **Note:** the phase-0 warning that the debug client does not reliably host headless JS did not bite here (Metro was running); a release-style build is still the safer check for the receive branch.
